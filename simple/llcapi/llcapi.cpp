...
int CompileLLtoASFile (const char * infile, const char* outfile, LLVMTypeMachine f) {
  ...
  for (unsigned I = TimeCompilations; I; --I)
    if (int RetVal = compileModule(argv[0], Context))
      return RetVal;
  return 0;
}
...
static int compileModule(const char * argvn, LLVMContext &Context) {
...

  {
    ...
    if (!RunPass.empty()) {
      if (!StartAfter.empty() || !StopAfter.empty()) {
        errs() << argvn << ": start-after and/or stop-after passes are "
                             "redundant when run-pass is specified.\n";
        return 1;
      }
      const PassInfo *PI = PR->getPassInfo(RunPass);
      if (!PI) {
        errs() << argvn << ": run-pass pass is not registered.\n";
        return 1;
      }
      ...
      if (!StartAfter.empty()) {
        ...
        if (!PI) {
          errs() << argvn << ": start-after pass is not registered.\n";
          return 1;
        }
      ...
      if (!StopAfter.empty()) {
        ...
        if (!PI) {
          errs() << argvn << ": stop-after pass is not registered.\n";
          return 1;
        }
