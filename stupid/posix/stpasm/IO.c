...
// vystupni zarizeni pro hlaseni ERRRRORU :
...
static Symbol Form(int B, char *Name) {
   Symbol N = (Symbol)malloc(sizeof *N + B*sizeof(Symbol));
	if (N == 0) 
	{
		fprintf(errout, "Form() - Out of memory.\n");
		fflush (stdout);
		fflush (stderr);
		exit(1);
	}
...
void ERROR(const char *Format, ...) {
   va_list AP;
   if (InSeg) fprintf(errout, "%s: [%d] ", FileTab[StartF], StartLine);
   va_start(AP, Format);
   vfprintf(errout, Format, AP); fprintf(errout,"\n");
   va_end(AP);
   if (++ERRORS >= 24)
   {
	   fprintf(errout, "Too many errors.  Aborting.\n");
	   fflush (stdout);
	   fflush (stderr);
...
void FATAL(const char *Format, ...) {
   va_list AP;
	if (InSeg)
		fprintf(errout, "%s: [%d] ", FileTab[StartF], StartLine);
   va_start(AP, Format);
	vfprintf(errout, Format, AP); fprintf(errout,"\n");
   va_end(AP);
   fflush (stdout);
...
void CHECK(void) {
	if (ERRORS > 0){
		fprintf(errout, "Errors present.  Assembly stopped.\n");
		fflush (stdout);
		fflush (stderr);
		exit(1);
	}
...
void PutL(unsigned long L, FILE *FP) {
   char A = ((char)(L >> 24))&0xff, B = ((char)(L >> 16))&0xff, C = ((char)(L >> 8))&0xff, D = ((char)L)&0xff;
...
void *Allocate(unsigned Size) {
   void *X;
	if (Size == 0) return 0;
	X = malloc(Size); if (X == 0) FATAL("Allocate() - Out of memory.");
   return X;
}
...
void OpenF(char *Name) {
   ...
   if (Files >= FileMax) {
      ...
      FileTab = (char **)realloc(FileTab, FileMax * sizeof *FileTab);
		if (FileTab == 0) FATAL("OpenF() - Out of memory.");
	}
...
#define SEG_MAX 0x20
struct _Segment SegTab[SEG_MAX], *SegP;
unsigned long LOC;
 
#define GAP_MAX 0x100
struct _Gap GapTab[GAP_MAX], *GapP;
...
void Space(word Rel) {
   ...
   LOC += Rel;
   Addr = SegP->Base + LOC;
   if (!SegP->Rel && Addr > AdP->Hi + 1) FATAL("\nPrekrocena maximalni velikost programu!");
}
...
static Lexical FindKey(char *Name) {
   Code *CP; Card *C;
   static const Card *EndR = Reserved + ELEMENTS(Reserved);
...
typedef struct _ValCard {
   char *Name; byte Type; word Val;
} ValCard;
static const ValCard ValTab[] = {
/* Special function registers */
   { "AR0", DATA, 0x00 }, { "AR1", DATA, 0x01 },
   { "AR2", DATA, 0x02 }, { "AR3", DATA, 0x03 },
   { "AR4", DATA, 0x04 }, { "AR5", DATA, 0x05 },
   { "AR6", DATA, 0x06 }, { "AR7", DATA, 0x07 },
   { "P0", SFR, 0x80 }, { "P1", SFR, 0x90 },
   { "P2", SFR, 0xa0 }, { "P3", SFR, 0xb0 },
   { "P4", SFR, 0xc0 }, { "P5", SFR, 0xc4 },
   ...
   { "TL2", SFR, 0xec }, { "TH2", SFR, 0xed }, { "TH3", SFR, 0xff },
   { "SP",   SFR, 0x81 }, { "DPL",  SFR, 0x82 }, { "DPH",  SFR, 0x83 },
   { "PSW",  SFR, 0xd0 }, { "ACC",  SFR, 0xe0 }, { "B",    SFR, 0xf0 },
   { "PWMP",  SFR, 0xfe }, { "PWM0",  SFR, 0xfc }, { "PWM1",    SFR, 0xfd },
...
void RegInit(void) {
   const ValCard * V; Symbol Sym;
   static const ValCard * const EndV = ValTab + ELEMENTS(ValTab);
...
Lexical OldL;
#define Ret(Token) return (*U = '\0', OldL = (Lexical) (Token))
