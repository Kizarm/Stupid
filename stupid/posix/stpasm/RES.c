...
void Resolve(Item IP) {
   Exp E = IP->E; byte Code; long PC; Segment Seg=0;
   ...
   switch (IP->Tag) {
      ...
      break;
      case 'L':
         if ( L >= 0x10000) ERROR("Code address out of range.");
      break;
...
void Reloc(byte Code, byte Tag, Exp E)
{
	...
	{
		RMax += R_EXTEND;
		RTab = (Item)realloc(RTab, RMax * sizeof *RTab);
		if(RTab == 0)
		{
			FATAL("Reloc() - Out of memory.");
		}
	}
	IP = &RTab[RCur];
