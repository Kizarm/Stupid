#ifndef _I8051PROCESSOR_H
#define _I8051PROCESSOR_H
#include "procesor.h"

//struct FILE;

class I8051Processor : public BaseProcessor {
  public:
    I8051Processor() : BaseProcessor() {};
    virtual ~I8051Processor() {};
    /*---------------------------------------------------------------------------*/
    /*   Pragma                                                                  */
    /*---------------------------------------------------------------------------*/
    ...
    void c_spar(unsigned long int a);    /*  adr operandu na stacku real - wrap pro c_spa  */
  ...
  protected:
    ...
    void rcall (int n);
    void rjump (int n);
    void FileAppend (FILE * to,FILE * from);
    void  ASp (int a);
    void  LSp (int a);
    int   LTos(void);
    int  uber  (void);
    ...
    void pop_b (void);
    void push_a (void);
    void c_lit (unsigned long a);
    void PushShortLbl(void);
    void PushLongLbl(void);
    void PopLbl(void);
    void porovimm(void);
    int ConvAToOff (long int a);
    void SpecVar (unsigned long int n);
    void SpecBit (unsigned long int n);

  private:
    FILE  /** fi,*/*tf,*pf;
    short  Stack[256];
    int    HLSt,HASt,Soff,Sp,LReg;
    int    MaxSp;
    int    LblNum,TxtNum,ProcNum,FuncNum ;
    int    FlagLJ;

    long int  CSize,LSize,TSize ;
    char t[80];

};

#endif //  _I8051PROCESSOR.H
