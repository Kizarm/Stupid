...
CLoaderDlg::CLoaderDlg (CPesNet * net, MainWindow * w) : QThread (w) {
  m_nBlok = NULL;
  m_nStAddr = 31;
  m_bRegistred = false;
  m_bSTOP = false;
  ...
  connect(this, SIGNAL(sMessageBox(QString,int)), this, SLOT(rMessageBox(QString,int)));
}
void CLoaderDlg::run (void) {
  bool result = false;
  do {
    if (!InitPES ())    break;
    if (!GetPESInfo())  break;
    switch (lastOperation) {
      ...
      case OperationVerify:
        result = Verifi  (m_nStartAddress, m_nCodeLen);
        break;
      default: break;
    }
    if (!result) break;
    if (lastOperation == OperationDownload) {
      if (!Reboot()) break;
    }
    result = true;
  } while (0);
  if (result) Status ("Operation %d OK.", lastOperation);
  m_OperationResult = result;
}

void CLoaderDlg::AfxMessageBox (const char * msg, int flags, void *) {
  qDebug("AfxMessageBox:%s", msg);
  QString str = QString::fromUtf8(msg);
  emit sMessageBox(str, flags);
  return;       // dalsi zrejme neni thread safe
}
void CLoaderDlg::rMessageBox (const QString msg, int flags) {
  unsigned butt = 0;
  /*if (flags & XB_OK)*/        butt |= QMessageBox::Ok;
  if (flags & XB_ICONSTOP)      butt |= QMessageBox::Critical;
  if (flags & XB_ICONWARNING)   butt |= QMessageBox::Warning;
  QMessageBox msgBox;
  msgBox.setText (msg);
  //msgBox.setInformativeText (str);
  msgBox.setStandardButtons ((QMessageBox::StandardButton) butt);
...
bool CLoaderDlg::Verify (const char * filename) {
  lastOperation = OperationVerify;
  m_strFilename = filename;
  bool result = false;
  if (!ReadDNLFile()) return result;
  result = true;
  start();
  return result;
}
bool CLoaderDlg::Download (const char * filename) {
  lastOperation = OperationDownload;
  m_strFilename = filename;
  bool result = false;
  if (!ReadDNLFile()) return result;
  result = true;
...
bool CLoaderDlg::Reboot() {
  ...
  if (! m_PES->SendCmd (Cm_IO_Reboot)) return false;
...
//CStringArray  DnlLines;
  ...
  /* debug 
  FILE * out = fopen ("hex.hex","w");
  fprintf (out, "m_nStAddr=%d, m_nDataBegin=0x%04lX, m_nDataSize=%ld, m_nStartAddress=0x%04X\n",
          m_nStAddr, m_nDataBegin, m_nDataSize, m_nStartAddress);
  fprintf (out, "m_nCodeLen=%ld\n", m_nCodeLen);
  for (i=0,k=0; i<m_nCodeLen; i++) {
    fprintf (out, "%02X", m_nBlok[i]); k++;
    if (k==16) {
      fprintf (out, "\n"); k=0;
    }
  }
...
bool CLoaderDlg::InitPES() {
  ...
  if (!m_PES->Success) {
    ...
    if (!m_PES->Success) {
      ...
      err.UnlockBuffer();
      AfxMessageBox (err, XB_OK|XB_ICONSTOP, NULL);
...
*/
      ...
      if (m_PES->StationNum == 0) {
        AfxMessageBox ("Chyba : v síti nebyla detekována žádná stanice!", XB_OK|XB_ICONSTOP, NULL);
        return false;
      }
  ...
  if (m_PES->IOData[0] != 0x5A) {
    ...
    AfxMessageBox (str, XB_OK|XB_ICONSTOP, NULL);
    return false;
  }
...
bool CLoaderDlg::GetPESInfo() {
  ...
  while (rep > 0) {
    ...
    if (!m_PES->ReadEEByte (EE_CodeMemCfg, pom)) {
      AfxMessageBox ("Chyba : Nelze číst z EEPROM", XB_OK|XB_ICONSTOP, NULL);
      return false;
    }
    fseries = false;
    switch (pom) {
      ...
      default:
        err = true;
//      AfxMessageBox( "Chyba : Tato paměťová konfigurace není podporována!", XB_OK|XB_ICONSTOP, NULL);
//      return false;
    }
    --rep;
  }
  ...
  if (nCodeMem < m_nCodeLen) {
    AfxMessageBox ("Chyba : Program je příliš dlouhý!", XB_OK|XB_ICONSTOP, NULL);
    return false;
  }
  ...
  if ( (nDataMem < (m_nDataBegin + m_nDataSize)) && m_nDataSize != 0) {
    AfxMessageBox ("Chyba : Program obsahuje příliš mnoho deklarovaných proměnných!", XB_OK|XB_ICONSTOP, NULL);
    return false;
  }
  return true;
}
  ...
  if (lock) {
    ...
    if (IsPassword()) {
      ...
      if (! (entredpsw.GetLength() == 5 && entredpsw[0] == 'A' && entredpsw[1] == 'D' &&
          ...
          entredpsw != psw) {
        // pokud nesedi ==> konec
        AfxMessageBox ("Chyba : Chybné heslo!", XB_OK|XB_ICONSTOP, NULL);
        return false;
      }
    }
    ...
    if (!m_bAutoUnlock && !m_bNoLock) {
      ...
      if (AfxMessageBox ("Programová paměť je uzamčena!\nPřejete si pokračovat?", XB_ICONSTOP|XB_YESNO) == IDNO) {
        // pokud ne ukonci to
        return false;
      }
    }
...
*/
  ...
  while (i < m_nCodeLen) {
    ...
    // zapis bloku do RAM
    TRACE ("Zapis stranky %d o velikosti %ld do RAM\n", (i / m_nBlockSize), size);
    unsigned short pageStart = (unsigned short) m_nVirtualPageStart << 8U;
    bError = m_PES->DownloadBlock (pageStart, size, pBuffer);
    if (bError != 16) { // chyba
      TRACE ("\tERROR\n");
      if (rep--) {
        continue;
        ...
        AfxMessageBox (csMessage, XB_OK|XB_ICONSTOP, NULL);
        return false;
      }
    }
    ...
    bError = m_PES->WriteEEByte (0xFF, m_nPageBegin + (BYTE)(i/m_nBlockSize));
    if (bError != 0x5A) {
      if (rep--) {
        continue;
      } else {
...
//    if (m_PES->ComPort != 0)
    {
      ...
      if (!Verifi (m_nStartAddress + i, size)) {
        ...
        if (rep--) {
          ...
          AfxMessageBox ("Chyba : Chybna verifikace!", XB_OK|XB_ICONSTOP, NULL);
          return false;
        }
      }
    }
...
*/
  ...
  if (!m_PES->WriteEEByte (EE_SizeLo, (BYTE) (m_nCodeLen%256))) {
    ...
    AfxMessageBox ("Chyba : Nelze zapsat do EEPROM", XB_OK|XB_ICONSTOP, NULL);
    return false;
  }
...
bool CLoaderDlg::SaveInfo() {
  ...
  for (i = 0; i < 8; i++) dato[i] = 0;
  ReportInfo ("SaveInfo ... nazev souboru\n");
  QFileInfo fi(m_strFilename.data());
  if (!fi.exists()) return false;
  QString base = fi.baseName();
  if (base.isEmpty()) base = "default";
  strncpy (reinterpret_cast<char*>(dato), base.toLatin1().constData(), 9);
  /*
  // BUG:!!! chyba ve win - Uložení názvu DNL souboru  C:\zmrsenina
  ...
  if (!l) {
    ...
    if (!m_bNoLock) {
      if (!m_PES->WriteXBlock (sekvence, 0x0b00, 18)) {
        AfxMessageBox ("Chyba : Nelze odemknout programovou paměť!", XB_OK|XB_ICONSTOP, NULL);
        return false;
    ...
    if (!m_PES->WriteEEByte (27, 0xaa)) {
      AfxMessageBox ("Chyba : Nelze uzamknout programovou paměť!", XB_OK|XB_ICONSTOP, NULL);
      return false;
    }
