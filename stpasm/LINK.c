...
void GetHead(FileBuf F) {
   ...
   if (GetL(FP) != ((unsigned)S&0xffffffffu)) FATAL("Corrupt object file %s.", F->Name);
   ...
   for (S = 0; S < F->Files; S++) {
      word L;
      L = GetW(FP), zahod = fread(Buf, 1, L, FP), Buf[L] = '\0';
      F->File[S] = CopyS((char*)Buf);
   }
   for (S = TYPES; S < F->Segs; S++) {
...
void AddReloc(byte Size, long PC, long Val) {
   RList R = (RList) Allocate(sizeof (struct _RList));
   RList P, S;
...
void SetRelocs(FileBuf F) {
   long S; struct _Item IBuf; Exp E, NextE;
   FILE *FP = fopen(F->Name, "rb+");
   if (FP == 0) FATAL("Cannot open %s.", F->Name);
...
void EndHex(void) {
   if (HexX > 0) {
      ...
      fprintf(OutF, ":%02X%04lX00", HexX, HexAddr);
      for (H = 0; H < HexX; H++)
...
void PutHex(long Addr, byte Hex) {
   ...
   if (HexX == 0x10) {
      int H, Sum;
      Sum = (HexAddr&0xff) + ((HexAddr >> 8)&0xff) + HexX;
      fprintf(OutF, ":10%04lX00", HexAddr);
      for (H = 0; H < HexX; H++)
         fprintf(OutF, "%02X", HexBuf[H]), Sum += HexBuf[H];
      fprintf(OutF, "%02X\n", (-Sum)&0xff);
      HexX = 0;
