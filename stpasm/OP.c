...
void OpInit (void) {
  ...
  for (OC = CodeTab, M = ModeTab; OC < CodeTab + ELEMENTS (CodeTab); OC++) {
    ...
    if (M > ModeTab + ELEMENTS (ModeTab)) {
      fprintf (errout, "Bad opcode initialization.\n");
      fflush (stdout);
      fflush (stderr);
      exit (1);
    }
...
void ParseArgs (byte Mnem) {
      ...
      case AT:
        ...
        switch (L) {
          case REGISTER:
            switch ( (Register) Value) {
              case ACC:
                if (Scan() != PLUS)
                  ...
                  switch (Value) {
                    case ACC:
                      ERROR ("Cannot use @A+A"), *XP = ' ';
                      break;
                    case AB:
                      ERROR ("Cannot use @A+AB.");
                      *XP = ' ';
                      break;
                    case CY:
                      ERROR ("Cannot use @A+C.");
                      *XP = ' ';
                      break;
                    case DPTR:
                      *XP = '>';
              ...
              default:
                Value -= (short) R0;
                if (Value != 0 && Value != 1) {
                  ERROR ("Register in @Rn out of range.");
                  *XP = ' ';
            ...
            break;
          default:
            ERROR ("Register must appear after @.");
            *XP = ' ';
            break;
        }
        break;
  ...
  *XP = 0, *EP = 0;
  ...
  if (MP >= CP->Start + CP->Modes) {
    ERROR ("Invalid addressing mode: %s.", CP->Name);
    return;
  }
  Op = MP->OpCode;
  S = MP->Y, EP = XExp;
  switch (*S) {
    case 'n':
      S++;
      Op |= XReg&7;
      break;/* Register R0/R1/R2/R3/R4/R5/R6/R7 */
    case 'i':
      S++;
      Op |= XReg&1;
      break;/* Register Pointer @R0/@R1 */
    case 'x':
      E = EP[0], EP[0] = EP[1], EP[1] = E;
      S++;
      break;
    case 'P':
      Reloc (Op, 'P', *EP++);
      return; /* P Paged Address */
  }
  PByte (Op);
