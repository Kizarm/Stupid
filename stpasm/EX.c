...
static struct _Exp EStack[STACK_MAX], *EP;
...
Exp EvalExp(Exp E) {
   ExpTag Tag = E->Tag;
   Symbol Sym; Lexical Op; Exp A, B, C, E1=0; byte H=0, Bs;
   Segment Seg; word Offset, Value=0, vB;
   if (Phase == 1) {
      if (E->Mark) return E;
...
//       OP(E) = va_arg(AP, Lexical), ARG1(E) = va_arg(AP, Exp);
         OP(E) = (Lexical) va_arg(AP, int), ARG1(E) = va_arg(AP, Exp);
      break;
      case BinX:
//       OP(E) = va_arg(AP, Lexical), ARG1(E) = va_arg(AP, Exp),
         OP(E) = (Lexical) va_arg(AP, int), ARG1(E) = va_arg(AP, Exp);
...
#define bO 0x80
static const int OpTab[] = { /* Bits 0-3: precedence, Bit 4:unary, Bits 5-7: operator type */
...
Exp Parse(int Dir) {
   Lexical L = OldL; Exp A, B; int Act;
   InExp = 1; Direct = Dir;
...
END_EX:
   ...
   switch (Act) {
	  ...
	  default : FATAL("Vnitrni chyba"); goto END_EX;
   }
}
